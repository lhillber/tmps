...
u0 = 1.257e-6 * 1e2 * 1e-12 # kg m A^-2 s^-2 cm/m s^2/us^2
m  = 1.16e-26 # kg
mu = 9.274e-24 * 1e4  # A m^2 cm^2/ m^2
kB = 1.381e-23 * 1e4 * 1e-12 * 1e-3 # kg m^2 s^-2 K^-1 cm^2/m^2 s^2/us^2 K/mk
h  = 6.62607004e-34 * 1e4 * 1e-6  # m^2 kg s^-1 cm^2/m^2 s/us

# PARAMETERS
# ==========
# radius of slower tube
rtube = 0.75 * 2.54 / 2  # cm
...
params = dict(
    ...
    Npulse   = 3,                           # Number of current pulses
...
def plot_phase_space(fignum, cloud, time_indicies=[0, -1]):
    ...
    for i, (coord, letter) in enumerate(
        ...
        for j, (ti, label) in enumerate(zip(time_indicies, ['initial', 'final'])):
            ...
            # the scatter plot:
            axScatter.scatter(x, y, s=0.25, label=label, rasterized=False,
            alpha=0.5)
...
def plot_tmps(fignum, cloud, include_names=['Tx','Ty','Tz','T']):
    ts = cloud.ts
    tmps = cloud.tmps
    ...
    colors = ['C3', 'C7', 'k', 'C2']
    lines = ['--','-.', '-',':']
    for label, temp, color, line in zip(temp_names, tmps.T, colors, lines):
...
def plot_scalar_summary(fignum, cloud):
    ...
    axD.legend()
    for label, temp in zip(temp_names, tmps.T):
...
# ============================
class Cloud():
    def __init__(self, Natom, T, width, r0_cloud, suffix,
    ...
    def save(self):
        # cloud.bin
        print('saving simulation data to {}'.format(self.name+'.txt'))
        file = open(self.name + self.suffix + '.bin','wb')
        file.write(pickle.dumps(self.__dict__))
        file.close()

    # load state...
    def load(self):
        print('loading simulation data from {}'.format(self.name+'.txt'))
        file = open(self.name + self.suffix + '.bin','rb')
...
def maxwell_velocity(T, seed=None, nc=3):
    np.random.seed(seed)
    if nc == 1:
...
def make_acceleration(curr_pulse, xyz, grad_norm_B):
    x, y, z = xyz
    dBdx,dBdy, dBdz = grad_norm_B
    def a(xs, t):
        xinterp = RegularGridInterpolator((x,y,z), dBdx,
        ...
        a_xyz = - mu / m * curr_pulse(t, **params) * np.c_[
...
# Default behavior
def run_sim(params,
        load=False, recalculate_B=False, resimulate=True, replot=True):
    ...
    if not hasattr(cloud, 'traj') or resimulate:
        ...
        a = make_acceleration(curr_pulse, cloud.xyz, cloud.grad_norm_B)
        ...
        for p in range(params['Npulse']):
            for t in np.arange(ta, tb, params['dt']):
                ...
                if ta + params['tau'] - params['dt']/2 < t < ta + params['tau'] + params['dt']/2:
                    ...
                    print( '----------------------------')
                    for label, temp in zip(cloud.temp_names, cloud.tmps.T):
    ...
    if not hasattr(cloud, 'traj') or\
        ...
        fignum = plot_tmps(fignum, cloud, include_names=['Tx','Ty','Tz'])
